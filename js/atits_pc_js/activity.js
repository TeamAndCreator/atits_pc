...
//判断当前用户角色，决定是否添加添加删除框
    if (rolesId.indexOf(1) != -1||rolesId.indexOf(3) != -1) {
        $('#zdhd').html("<button class=\"btn btn-success\" data-toggle=\"modal\" data-target=\"#demo-lg-modal\">\n" +
...
//删除
    $('#delete').click(function () {
        ...
        $.ajax({
            type: 'post',
            dataType: 'JSON',
            url: ipValue + '/activity/deleteByIds',
...
function invoiceFormatter(value, row) {
    return '<a href="activity_detail.html?id='+row.id+'" class="btn-link" >' + value + '</a>';
}
...
function updateState(value, activityId) {
    if (value == 0) {
        $('#sh').text("是否通过");
    }
    $('#tg').click(function () {
        $.ajax({
            type: 'post',
            dataType: 'JSON',
            url: ipValue + '/activity/updateState',
            data: {_method: "put", "id": activityId, "state": 1},
            async: false,
    ...
    $('#wtg').click(function () {
        $.ajax({
            ...
            dataType: 'JSON',
            url: ipValue + '/activity/updateState',
            data: {_method: "put", "id": activityId, "state": 2},
            async: false,
            success: function (data) {
                window.location.reload()
            },
