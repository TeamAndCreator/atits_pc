...
//判断当前用户角色，决定是否添加添加删除框
    if (rolesId.indexOf(1) != -1||rolesId.indexOf(3) != -1) {
        $('#zdwjbg').html("<button class=\"btn btn-success\" data-toggle=\"modal\" data-target=\"#demo-lg-modal\">\n" +
...
//删除
    $('#delete').click(function () {
        ...
        $.ajax({
            type: 'post',
            dataType: 'JSON',
            url: ipValue + '/report/deleteByIds',
...
function invoiceFormatter(value, row) {
    return '<a href="report_detail.html?id='+row.id+'" class="btn-link" >' + value + '</a>';
}
...
function updateState(value, reportId) {
    if (value == 0) {
        $('#sh').text("是否通过");
    }
    $('#tg').click(function () {
        $.ajax({
            type: 'post',
            dataType: 'JSON',
            url: ipValue + '/report/updateState',
            data: {_method: "put", "id": reportId, "state": 1},
            async: false,
    ...
    $('#wtg').click(function () {
        $.ajax({
            ...
            dataType: 'JSON',
            url: ipValue + '/report/updateState',
            data: {_method: "put", "id": reportId, "state": 2},
            async: false,
            success: function (data) {
                window.location.reload()
            },
